    ...
    <body class="antialiased">
        ...
        <main>
            <div class="container mt-4">
                <a href="{{ url('/dashboard') }}" class="btn btn-primary btn-lg">Back</a>
                <h1 class="pizzas text-center p-4">Orders</h1>
                    ...
                    <div class="col-lg-8 ">
                        <table class="table table_style" >
                            ...
                            <tr>
                                ...
                                <th scope="col">#</th>
                                @endif
                                <th scope="col" class="th_style">User</th>
                                <th scope="col" class="th_style">Pizza</th>
                                <th scope="col" class="th_style">Pizza size</th>
                                <th scope="col" class="th_style">Pizza attachments</th>
                                <th scope="col" class="th_style">Price</th>
                            </tr>
                            </thead>
                            <tbody>
                            ...
                            <tr class="tr_style">
                                @if (auth()->user()->roles==1)
                                <th scope="row">{{ $orders2->id }}</th>
                                @endif
                                @foreach ( $users as $user)
                                @if ($user->id == $orders2->user_id)
                                <td class="th_style">{{$user->name }}</td>  
                                @endif
                                @endforeach
                                @foreach ( $pizzas as $pizza)
                                @if ($pizza->id == $orders2->pizza_id)
                                    <td class="th_style">{{$pizza->pizza_name }}</td>  
                                @endif
                                @endforeach
                                @foreach ( $sizeprices as $sizeprice)
                                @if ($sizeprice->id == $orders2->pizza_size_price_id)
                                    <td class="th_style">{{$sizeprice->pizza_size }}</td>  
                                @endif
                                @endforeach
                                @foreach ( $attachments as $attachment)
                                @if ($attachment->id == $orders2->pizza_attachments_id)
                                    <td class="th_style">{{$attachment->attachment_name }}</td>  
                                @endif
                                @endforeach
                                <td class="th_style">{{$orders2->price }}</td>  
                            </tr>
                            @endforeach
                            </tbody>
                        </table>
