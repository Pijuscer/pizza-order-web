...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
        ...
        <main>
            <div class="container container_size_price mt-4">
                <div class="d-flex justify-content-center">
                  <div class="col-md-10">
                    @if (session('message_sizes_prices_add'))
                      <div class="alert alert-success">{{session('message_sizes_prices_add')}}</div>
                    @endif
                    <h1 class="size_price text-center p-4">Add size and price</h1>
                      @if ($errors->any())
                      ...
                      @endif
                      <form action="sizes_prices" method="POST" class="row g-3 transboxaboutadd size_price_form">
                        @csrf
                        <div class="col-md-6 label_input">
                            <label for="pizza_size" class="form-label add_label_text">Pizza size</label>
                            <input value="{{ old('pizza_size') }}" type="text" class="form-control" id="pizza_size" name="pizza_size" placeholder="Add pizza size">
                        </div>
                        <div class="col-md-6 label_input">
                            <label for="pizza_price" class="form-label add_label_text">Pizza price</label>
                            <input value="{{ old('pizza_price') }}" type="text" class="form-control" id="pizza_price" name="pizza_price" placeholder="Add pizza price">
                        </div>
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end button_save">
                            <button type="submit" class="btn btn-success btn-lg">Save</button>                 
                            <a href="{{ url('/all_sizes_prices') }}" class="btn btn-success btn-lg">All pizza size and price</a>
                        </div>
                      </form>
                    </div>           
                  </div>
